...
function getJsApiParameters($data=array()){
	...
	$payConfig=M('wechatpay_set')->where(array('token'=>get_token()))->find();
	//使用jsapi接口
	$jsApi = new JsApi_pub($payConfig['wxappid'],$payConfig['wxmchid'],$payConfig['wxkey'],$payConfig['wxappsecret']);
	...
	//使用统一支付接口
	$unifiedOrder = new UnifiedOrder_pub($payConfig['wxappid'],$payConfig['wxmchid'],$payConfig['wxkey'],$payConfig['wxappsecret']);
...
function sign_link($openid){
    $param=array(
        'openid'=>$openid
        
    );
    
    $sign_count=M('sign')->where($param)->count();
    $sign_info=M('sign')->where($param)->order('id DESC')->select(); 

    for($i=1;$i<=$sign_count;$i++){
    $nowtime=date('Y-m-d',time());
    $sign_day=date("Y-m-d",strtotime("$nowtime   -$i   day"));
    $date_arr=array(
        'openid'=>$param['openid'],
        'date'=>$sign_day
        
    );
    $date_for_count=M('sign')->where($date_arr)->find();
    if(empty($date_for_count)){
        $sign_day_link=$i;
        break;
        }
    }  
    return $sign_day_link; 
}

function sign_ranking($param){
...
function wxauth_unionid($url){
        $code=$_GET['code'];
        define('URL',  urlencode($url));
	...
	if(!empty($access_info['errcode'])){
		header("Location: https://open.weixin.qq.com/connect/oauth2/authorize?appid=wxf645e5f39d850ad8&redirect_uri=".URL."&response_type=code&scope=snsapi_base&state=STATE#wechat_redirect");
		exit;
		}
	...
	if(!empty($user_info['errcode'])){
		header("Location: https://open.weixin.qq.com/connect/oauth2/authorize?appid=wxf645e5f39d850ad8&redirect_uri=".URL."&response_type=code&scope=snsapi_userinfo&state=STATE#wechat_redirect");
		exit;
		}
