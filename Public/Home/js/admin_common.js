	...
	if(data.status){
		...
		if(is_mult){
			$addImg = $('<div class="upload-pre-item22"><img width="100" height="100" src="' + src + '"/>'
				+'<input type="hidden" name="'+name+'[]" value="'+data.id+'"/><em>&nbsp;</em></div>');
			$("#mutl_picture_"+name).append($addImg);
			
			$('.upload-pre-item22 em',$addImg).click(function() {
				if(confirm('确认删除？')){
					$(this).parent().remove();
				}
			});
...
(function(){
	...
	*/
	...
	/* 选择图文素材 */
	function openSelectAppMsg(dataUrl,callback){
		...
		$.Dialog.open("选择图文素材",{width:1000,height:640},$contentHtml);
		$.ajax({
			url:dataUrl,
			data:{'type':'ajax'},
			dataType:'html',
